...
def upload_documento(id):
    # (Este código não foi alterado internamente, apenas o decorador)
    funcionario = Funcionario.query.get_or_404(id)
    if funcionario.empresa_id != current_user.empresa_id:
        flash('Acesso negado.', 'danger')
        return redirect(url_for('rh.listar_funcionarios'))
...
def uploaded_file(empresa_id, funcionario_id, filename):
    # (Este código não foi alterado internamente, apenas o decorador)
    # Acesso a uploads deve ser verificado com base na permissão RH também?
    # Por segurança, vamos adicionar o decorador aqui também.
    # Se o usuário não tiver acesso RH, ele não poderá ver a página do funcionário
    # e consequentemente não chegaria aqui, mas é uma camada extra.
    # No entanto, a rota original não tinha o decorador, vamos manter assim por enquanto
    # e confiar que o acesso à página que contém o link já foi validado.
...
def importar_funcionarios():
    ...
    try:
        ...
        for row in sheet.iter_rows(min_row=2, values_only=True):
            ...
            try:
                # Tenta analisar datas assumindo que podem ser objetos datetime vindos do Excel
                data_nasc_raw = data.get('data_nascimento')
                data_adm_raw = data.get('data_admissao')

                if isinstance(data_nasc_raw, datetime):
                    data_nasc = data_nasc_raw.date()
                else:
                     data_nasc = datetime.strptime(str(data_nasc_raw).split(' ')[0], '%Y-%m-%d').date()

                if isinstance(data_adm_raw, datetime):
                    data_adm = data_adm_raw.date()
                else:
                    data_adm = datetime.strptime(str(data_adm_raw).split(' ')[0], '%Y-%m-%d').date()
            ...
            novo_func = Funcionario(
                nome=data.get('nome'),
                cpf=str(data.get('cpf')), # Garante que CPF seja string
                rg=str(data.get('rg', '')),
...
@login_required
# Não precisa de decorador específico, qualquer usuário logado pode baixar o template
def download_template():
    # (Este código não foi alterado internamente)
    output = BytesIO()
    ...
    # Adiciona formato de data esperado
    sheet['D2'] = '1990-05-15'
    sheet['E2'] = '2025-08-01'
    ...
    example = [
        "João da Silva", "123.456.789-00", "joao.silva@exemplo.com", sheet['D2'].value, sheet['E2'].value,
        "3500.50", "Analista Financeiro", "Financeiro", "1234567", "M", "Casado(a)",
        "(51) 99999-8888", "Rua das Flores, 123", "90000-000", "Porto Alegre", "RS"
    ]
    # Limpa as células de data após pegar o valor formatado
    sheet['D2'] = None
    sheet['E2'] = None
    sheet.append(example) # Adiciona a linha de exemplo
...
def novo_cargo():
    ...
    if request.method == 'POST':
        ...
        novo_cargo = Cargo(
            ...
            descricao=request.form['descricao'],
            salario_base=float(salario_str) if salario_str else 0.0,
...
def editar_cargo(id):
    ...
    if request.method == 'POST':
        ...
        cargo.salario_base = float(salario_str) if salario_str else 0.0
